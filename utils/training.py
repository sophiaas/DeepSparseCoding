...
def train_mod(data, params, schedule):
    ## Import model
    model = mp.get_model(params['model_type'], params, schedule)
...
def infer_coeffs(data, params, schedule):
    ## Import model
    model = mp.get_model(params["model_type"], params, schedule)
    coeffs = np.zeros((model.get_sched("num_batches")*model.batch_size, params["num_neurons"]))
    ...
    with tf.Session(graph=model.graph) as sess:
        ...
        for b_step in np.arange(model.get_sched("num_batches")):
