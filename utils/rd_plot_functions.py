...
def full_comparison_plots(lca_table, size=8, alt_1=None, alt_2=None, alt_3=None, alt_4=None, x='entropy', y='mse', ylim=.4, xlim=5, row='n_neurons', col='cost', hue='lambda', title=None):
#     if hue == 'lambda':
#         labels_key = make_legend_labels(lca_table)
    p = sns.lmplot(x, y, data=lca_table, size=size, row=row, col=col,  hue=hue, fit_reg=False, scatter_kws={"s": 3})
...
#             a.legend(handles, labels)
        if alt_1 is not None:
            a.plot(alt_1[x], alt_1[y], marker=".", color="black", linestyle="dashed")
#             a.plot(alt_1[x], alt_1[y].where(alt_1.n_bins>1), marker=".", color="black", linestyle="dashed")
        if alt_2 is not None:
            a.plot(alt_2[x], alt_2[y], marker="+", color="black", linestyle="dotted") 
...
def plot_bases(weights, padding=None):
    ...
    plot_data = pf.pad_data(weights.T.reshape((num_outputs, patch_edge_size,
    patch_edge_size)))
...
#         orig_img = dp.unwhiten_data(orig_img, D.data_mean, D.w_filter, method=params["whiten_method"])
#         w_filter = D.w_filter
#     reshape = (orig_img.shape[0], params['patch_edge_size'], params['patch_edge_size'], 1)
#     orig_img = np.reshape(orig_img, reshape) 
#     orig_img = dp.patches_to_image(orig_img, [25, params['im_edge_size'], params['im_edge_size'], 1])
#     orig_img = orig_img[0]
# #     orig_img, _, _ = dp.standardize_data(orig_img)
...
#     mean_coeff_range = np.mean(np.max(coeffs, axis=1) - np.min(coeffs, axis=1))
#     ## Generate reconstructions for quantized coeffs and plot
#     MSE = []; PSNR = []; MSSIM = []
#     for i, b in enumerate(params['bins']):
#         disc_coeffs, H, num_bins = rd.discretize_coeffs(coeffs, b, params, w_filter)
#         if params['model_type'] == 'rg':
...
#         recon0 = recon[0]
# #         recon, _, _ = dp.standardize_data(recon)        
#         axes[i+1].imshow(recon0.squeeze(), cmap="Greys_r")
#         mse = np.mean((orig_img - recon0)**2)
#         psnr = 'na'
#         struc_sim = ssim(orig_img.squeeze(), recon0.squeeze(), data_range=data_range)
# #         psnr = np.mean(10 * np.log10(np.max(orig_img) ** 2 / mse)) 
#         MSE.append(mse); PSNR.append(psnr); MSSIM.append(struc_sim)
#         axes[i+1].set_title('bins: ' + str(b) + '\n mse: ' + str(mse) + '\n psnr: ' + str(psnr) + '\n mssim: ' + str(struc_sim) + '\n transmission rate: ' + str(H  * params['n_neurons'] / 256))
#     stats = {'mse': MSE, 'psnr': PSNR, 'mssim': MSSIM}
#     return stats

def discretize_and_recon(params, plot=False, save_rd=True):
    ## Load in data and reconstruct whole image
    with np.load(params["input_dir"]) as d:
        D = d['arr_0'].item()['train']
        data = D.images
    ...
    if params["white"]:
        orig = dp.unwhiten_data(data, D.data_mean, D.w_filter, method=params["whiten_method"])
    else:
        orig = np.reshape(data, reshape) 
        orig = dp.patches_to_image(orig, [25, params['im_edge_size'], params['im_edge_size'], 1])
        orig += D.data_mean
    orig_img = orig[params['img_idx']]
    ...
    if params["model_type"] == 'pca':
        recon = bases.recon(weights, coeffs, 0)
    else:
    ...
    if params["white"]:
        recon = dp.unwhiten_data(recon, method=params["whiten_method"])
    else: 
        recon = np.reshape(recon, reshape)
        recon = dp.patches_to_image(recon, [25, params['im_edge_size'], params['im_edge_size'], 1])
    recon_img = recon[params['img_idx']]  
    axes[1].imshow(recon_img.squeeze(), cmap="Greys_r")
    # Generate reconstructions for quantized coeffs and plot
    MSE = []; PSNR = []; MSSIM = []
    for i, b in enumerate(params['bins']):
        disc_coeffs, H, num_bins = rd.discretize_coeffs(coeffs, b, params, bf_lengths)
        if params["model_type"] == 'pca':
            recon = bases.recon(weights, disc_coeffs, 0)
        else:
            recon = np.matmul(disc_coeffs, weights.T)
        recon = np.reshape(recon, reshape)
        recon = dp.patches_to_image(recon, [25, params['im_edge_size'], params['im_edge_size'], 1])
...
#         recon -= recon_mean
        ...
        if params["white"]:
            ...
            recon += D.data_mean
        recon_img = recon[params['img_idx']]
        axes[i+1].imshow(recon_img.squeeze(), cmap="Greys_r")
        mse = np.mean((orig_img - recon_img)**2)
        psnr = 10 * np.log10(data_range ** 2 / mse)
        struc_sim = ssim(orig_img.squeeze(), recon_img.squeeze(), data_range=data_range)
        MSE.append(mse); PSNR.append(psnr); MSSIM.append(struc_sim)
        axes[i+1].set_title('bins: ' + str(b) + '\n mse: ' + str(mse) + '\n psnr: ' + str(psnr) + '\n mssim: ' + str(struc_sim) + '\n transmission rate: ' + str(H  * params['n_neurons'] / 256))
    stats = {'mse': MSE, 'psnr': PSNR, 'mssim': MSSIM}
    return orig_img, recon


# def discretize_and_recon(params, plot=False, save_rd=True):
#     w_filter = None
#     bf_lengths = None
#     rate_distortion = np.array(["model", "n_neurons", "overcompleteness", "cost", "lambda", "p_active", "n_active", "mse", "mse_sd", "log_mse", "ssim", "entropy", "transmission_rate", "bin_width", "num_bins", "bin_ratio"], dtype="object")
#     ## Load in data and reconstruct whole image
#     with np.load(params["input_dir"]) as d:
#         D = d['arr_0'].item()['train']
#         data = D.images
# #     reshape = (data.shape[0], params['patch_edge_size'], params['patch_edge_size'], 1)
#     if params["white"]:
#         data = dp.unwhiten_data(data, D.data_mean, D.w_filter, method=params["whiten_method"])
#     else:
#         data = dp.reshape_data(data, flatten=False)[0]
#         data = dp.patches_to_image(data, [25, params['im_edge_size'], params['im_edge_size'], 1])
#         data += D.data_mean
# #     orig_img = orig_img[params['img_idx']]
#     data_range = np.max(orig_img, axis=1) - np.min(orig_img, axis=1)
#     ## Load in model coeffs and weights
#     if params['model_type'] == 'lca':
#         name = params['model_type']+'_'+str(params['n_neurons'])+'_'+params['cost']+'_'+str(params['lam'])+'_'+params['version']
#     else: 
